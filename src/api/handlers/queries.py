...
def create_query(query: Query, queries_operation: QueriesOperation = Depends(), authorize: AuthJWT = Depends(), need: Need = Depends(), timing: Timing = Depends()):  # add email from jwt token
